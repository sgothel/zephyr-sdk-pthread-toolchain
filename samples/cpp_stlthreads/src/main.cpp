...
int main()
{
  ...
  Bucket bucket;
  if (false) {
    tasks[0] = std::thread(&Bucket::ThreadConsumer, &bucket);
    tasks[1] = std::thread(&Bucket::ThreadProducer, &bucket);
  } else {
    tasks[0] = std::thread(&Bucket::ThreadProducer, &bucket);
    tasks[1] = std::thread(&Bucket::ThreadConsumer, &bucket);
  }
  dbg_PRINT("MAIN: p1 Bucket: %d/%d, taken %d, given %d", bucket.bucket_value, Bucket::ItemCount, bucket.bucket_taken, bucket.bucket_given);
  ...
  dbg_PRINT("MAIN: p1 Bucket: %d/%d, taken %d, given %d", bucket.bucket_value, Bucket::ItemCount, bucket.bucket_taken, bucket.bucket_given);
  if (Bucket::ItemCount != bucket.bucket_given || Bucket::ItemCount != bucket.bucket_taken || 0 != bucket.bucket_value)
  {
    src_ABORT();
  }
